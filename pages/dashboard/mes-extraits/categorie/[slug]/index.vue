...
const query = ref(
  `extract/getextracts/${route.params.slug}/${numberOfExtractToDisplay.value}`
)
...
async function next() {
  if (
    ...
    query.value = `extract/extract-cursor/${route.params.slug}/${numberOfExtractToDisplay.value}/${lastId.value}/1/desc`
...
async function previous() {
  if (lastId.value === null || lastNumberId.value === null) {
    ...
    query.value = `extract/extract-cursor/${route.params.slug}/${numberOfExtractToDisplay.value}/${firstId.value}/1/asc`
    const xsrfTokenTime = localStorage.getItem('xsrfToken_time')
    if (xsrfTokenTime !== null && Date.now() >= +xsrfTokenTime - 2000) {
      await refreshToken(config.public.baseURL)
      execute()
...
async function first() {
  query.value = `extract/${numberOfExtractToDisplay.value}`
  const xsrfTokenTime = localStorage.getItem('xsrfToken_time')
  if (xsrfTokenTime !== null && Date.now() >= +xsrfTokenTime - 2000) {
    await refreshToken(config.public.baseURL)
    execute()
...
onMounted(async () => {
  ...
  const resultLast = await lastNumberData(
    ...
    `extract/findLastExtract/${route.params.slug}`
  )
  lastNumberId.value = resultLast
  const resultFirst = await firstNumberData(
    config.public.baseURL,
    `extract/findFirstExtract/${route.params.slug}`
  )
