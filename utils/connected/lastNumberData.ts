...
export async function lastNumberData(baseURL: string) {
  ...
  const response = await fetch(
    `${baseURL}/promenadeditor/findLastPromenade`,
    options
  )
  if (response.ok) {
    ...
    const data = await response.json()
    await refreshToken(baseURL)
    await fetch(`${baseURL}/promenadeditor/findLastPromenade`, options)
    return data
  }
}
